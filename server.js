...
const productSchema = new mongoose.Schema({
    ...
    boughtInPastMonth: { type: String, required: true },
...
app.get('/api/products', async (req, res) => {
    try {
        ...
        const products = await Product.find(query).sort(sortOption).limit(parseInt(limit)).skip((parseInt(page) - 1) * parseInt(limit));
...
app.post('/api/products', upload.single('image'), async (req, res) => {
    try {
        ...
        if (!name || !price || !brand || !category) { return res.status(400).json({ message: 'Missing required fields.' }); }
        const newProduct = new Product({ ...req.body, imageUrl: req.file ? req.file.path.replace(/\\/g, "/") : '' });
...
app.delete('/api/products/:id', async (req, res) => {
    try {
        ...
        if (deletedProduct.imageUrl) {
            fs.unlink(path.join(__dirname, deletedProduct.imageUrl), err => { if (err) console.error("Error deleting image file:", err); });
        }
